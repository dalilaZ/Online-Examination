...
<nav>
  ...
  if(isset($_POST['login']) && isset($_POST['username']) && isset($_POST['password'])){
    ...
    //$who=$_POST['group1'];
  	...
  	$query= mysqli_query($connect,"SELECT * FROM student where user= '$username' and password='$password' ");
  	$xx = mysqli_fetch_array($query);
  	$user = $xx['user'];
  	$pass=$xx['password'];
	$teacher=$xx['is_teacher'];
    $whodb=$xx['type'];
  	if($username==$user && $password==$pass && $whodb=='teacher'){
  		$_SESSION['valid'] = true;
        $_SESSION['timeout'] = time();
        $_SESSION['username'] = $username;
        $_SESSION['group1'] = 'teacher';
          header("Location:index.php");
  	}
	else if($username==$user && $password==$pass && $whodb=='student'){
  		$_SESSION['valid'] = true;
        $_SESSION['timeout'] = time();
        $_SESSION['username'] = $username;
       $_SESSION['group1'] = 'student';
